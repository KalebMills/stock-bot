...
class ConfidenceScore {
    ...
    getConfidenceOptions(currTrade, changePercentPerMinute) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const relativeVolume = util_1._getRelativeVolume(this.ticker).then((data) => data);
            confidenceOptions.relativeVolume = {
                process: relativeVolume.then((vol) => !!(vol > 1)),
                score: relativeVolume.then((vol) => vol / 1 * 10)
            };
            const vwap = util_1.getTickerSnapshot(this.ticker).then((data) => (currTrade.p - data.day.vw) / data.day.vw);
            confidenceOptions.vwap = {
                process: vwap.then((increase) => !!(increase > 0)),
                score: vwap.then((increase) => Math.abs(increase) / 0.05 * 5)
            };
            const process = new Promise(() => changePercentPerMinute > 0);
