...
class LiveDataStockWorker extends StockWorker {
    ...
    */
    ...
    _getRelativeVolume(ticker) {
        const now = new Date();
        const todaysDate = util_1._convertDate(now);
        const lastDay = new Date();
        lastDay.setDate(util_1._returnLastOpenDay());
        const lastDate = util_1._convertDate(lastDay);
        const minutesPassed = util_1._minutesSinceOpen();
        let lastDayVolume = 0;
        let todaysVolume = 0;
        ...
        }).then((data) => { lastDayVolume = data.data.results.reduce((a, b) => a + b['v'], 0); }).catch(err => {
            throw new exceptions_1.InvalidDataError(`Error in ${this.constructor.name}._getRelativeVolume(): innerError: ${err} -- ${JSON.stringify(err)}`);
        });
        ...
        }).then((data) => { todaysVolume = data.data.ticker.day.v; }).catch(err => {
            throw new exceptions_1.InvalidDataError(`Error in ${this.constructor.name}._getRelativeVolume(): innerError: ${err} -- ${JSON.stringify(err)}`);
        });
