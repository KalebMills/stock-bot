...
const confidence_score_1 = require("./confidence-score");
;
/*
    ...
    */
    process(currTrade) {
        ...
        return this.datastore.get(ticker) //Fetch the previous quote
            ...
            if (!(data.length === 1)) {
                this.logger.log(base_1.LogLevel.INFO, `No data in datastore for ${ticker}`);
                ...
                this.logger.log(base_1.LogLevel.INFO, `${ticker} has changed ${changePercentPerMinute} per minute.`);
                //If the change percent is greater than .5% per minute, notify
                if (timeTaken >= 180) {
                    const confidence = new confidence_score_1.ConfidenceScore(ticker);
                    ...
                    return confidence.getConfidenceScore(yield confidenceOptions)
