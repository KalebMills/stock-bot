    ...
    process(currTrade) {
        ...
        return this.datastore.get(ticker) //Fetch the previous quote
            ...
            if (data.length !== 1) {
                ...
                this.logger.log(base_1.LogLevel.INFO, `${ticker} has changed ${changePercentPerMinute} per minute over ${timeTaken} minutes.`);
                //If the change percent is greater than .5% per minute, notify
                if (changePercentPerMinute > .01 && timeTaken >= 180) {
                    this.logger.log(base_1.LogLevel.INFO, `${ticker} has changed ${changePercentPerMinute} per minute. Checking confidence score..`);
                    const confidenceOptions = {
                        ...
                        'vwap': {
                            value: 5,
                            process: util_1.getTickerSnapshot(ticker).then(sn => (sn.day.vw > currTrade.p))
                        },
                        'totalVolume': {
                            value: 5,
