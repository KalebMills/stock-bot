...
class SocialMediaWorker extends StockWorker {
    process(input) {
        ...
        if (type === data_source_1.TwitterAccountType.SWING_POSITION) {
                    ...
                    returnPromise
                        ...
                        if (signal.action == util_1.ActionSignal.BUY) {
                            return Promise.all([this.exchange.sizePosition(ticker, this.accountPercent, signal.sizing), this.exchange.getBuyingPower()])
                                ...
                                const TOTAL_COST = new decimal_js_1.Decimal(qty * price).toNumber();
                                this.logger.log(base_1.LogLevel.INFO, `Buying ${qty} shares of ${ticker} for ${TOTAL_COST}. We have ${buyingPower} Buying Power`);
                            ...
                            return this.exchange.getPositionQty(ticker)
                                ...
                                return this.exchange.placeOrder({
                                    ...
                                    .catch(err => {
                                    this.logger.log(base_1.LogLevel.ERROR, util_2.inspect(err));
                                    throw err;
                                });
                            });
                        }
                        else {
                            this.logger.log(base_1.LogLevel.INFO, `Hit the else block for Action: ${signal.action}`);
                        }
                    });
                }
            }
        ...
        else if (type === data_source_1.TwitterAccountType.TRACKER) {
            for (let signal of signals) {
                let t = signal.ticker;
                this.metric.push({
                    'mentions': {
