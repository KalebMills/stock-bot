...
class LiveDataStockWorker extends StockWorker {
    ...
    */
    process(currQuote) {
        this.logger.log(base_1.LogLevel.INFO, `${this.constructor.name}:process(${JSON.stringify(currQuote)})`);
        return this.datastore.get(currQuote.sym) //Fetch the previous quote
            ...
            .then((data) => {
            if (!(data.length === 1)) {
                this.logger.log(base_1.LogLevel.INFO, `No data in datastore for ${currQuote.sym}`);
                //This is the first receive for a ticker, skip the analysis and just store this event in the DB
                return Promise.resolve();
            }
            else {
                const [prevQuote] = data;
                const changePercentPerMinute = this._getChangePercentPerMinute(currQuote, prevQuote);
                this.logger.log(base_1.LogLevel.INFO, `${currQuote.sym} has changed ${changePercentPerMinute} per minute.`);
                //If the change percent is greater than .5% per minute, notify
                if (changePercentPerMinute > .009) {
                    this.logger.log(base_1.LogLevel.INFO, `${currQuote.sym} has the required increase to notify in Discord`);
                    return this.notification.notify({
                        ...
                        return this.exchange.placeOrder({
                            ...
                            type: 'limit',
                            take_profit: {
                                limit_price: currQuote.ap + (currQuote.ap * .05),
                            },
                            stop_loss: {
                                stop_price: currQuote.ap - (currQuote.ap * .1),
                            }
                        })
                            .then(() => {
                            this.logger.log(base_1.LogLevel.INFO, `Place PAPER order for ${currQuote.sym}`);
                        });
                    })
                        .then(() => {
                    ...
                    this.logger.log(base_1.LogLevel.TRACE, `${currQuote.sym} did not meet the standard, it's changePerMinute = ${this._getChangePercentPerMinute(currQuote, prevQuote)}`);
                    return;
                }
            }
        })
    ...
    _getChangePercentPerMinute(currQuote, prevQuote) {
        this.logger.log(base_1.LogLevel.INFO, `currQuote: ${currQuote.ap} prevQuote: ${prevQuote.ap} -- currQuote.t = ${currQuote.t} --- prevQuote.t = ${prevQuote.t}`);
