    ...
    */
    get(data) {
        ...
        if (data.includes('*')) { //Wildcard search
            return Promise.reject(new exceptions_1.UnprocessableEvent('Unsupported wildcard functionality'));
            ...
            return new Promise((resolve, reject) => {
                this.client.hgetall(data, (err, d) => {
                    if (err) {
                        reject(err);
                    }
                    else {
...
class MemoryDataStore {
    ...
    save(key, data) {
        try {
            ...
            this.logger.log(base_1.LogLevel.TRACE, `${key} was saved into ${this.constructor.name}. Store now has ${Object.keys(this.store).length} entries in it`);
            this.logger.log(base_1.LogLevel.INFO, `${this.constructor.name}#store.length = ${Object.keys(this.store).length}`);
            this.metric.push({
                'memoryStoreKeys': {
    ...
    get(key) {
        this.logger.log(base_1.LogLevel.TRACE, `this.store.hasOwnProperty(${key}) == ${this.store.hasOwnProperty(key)}`);
        if (this._has(key) || this._hasWildCard(key)) {
            if (this._hasWildCard(key)) {
                return this._fetchWildCardValues(key);
            }
            else {
                this.logger.log(base_1.LogLevel.TRACE, `Found a single key: ${this.store[key]}`);
