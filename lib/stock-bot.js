...
class StockService extends base_1.Service {
    ...
    makeWorker(options) {
        return new StockServiceWorker(Object.assign(Object.assign({}, options), { exceptionHandler: this.exceptionHandler, postTransaction: this.postTransaction, purchaseOptions: this.purchaseOptions, exchange: this.exchange }));
    }
    close() {
...
class StockServiceWorker extends base_1.Worker {
    ...
    _processTicker(ticker) {
        return this.getPrevStockPrice(ticker.ticker, this.purchaseOptions.prevStockPriceOptions.unit, this.purchaseOptions.prevStockPriceOptions.measurement)
            ...
            if ((changePercent.percentChange >= .0005 && changePercent.persuasion === 'up') && (ticker.price <= this.purchaseOptions.maxSharePrice)) {
                this.logger.log(base_1.LogLevel.INFO, `We should buy ${ticker.ticker} at $${ticker.price}/share, previous price ${this.purchaseOptions.prevStockPriceOptions.unit} ${this.purchaseOptions.prevStockPriceOptions.measurement}s ago was $${prevStockPrice}/share`);
                ...
                return this.exchange.getBuyingPower()
                    .then(amount => {
                    this.logger.log(base_1.LogLevel.INFO, chalk_1.default.green(`Checking buying power.`));
                    const cost = this.purchaseOptions.maxShareCount * this.purchaseOptions.maxSharePrice;
                    if (cost < amount) {
                        //Purchase
                        this.logger.log(base_1.LogLevel.INFO, chalk_1.default.greenBright(`We have enough buying power, purchasing ${ticker.ticker}`));
                        return this.exchange.buy({
                            ...
                            stop_loss: {
                                limit_price: stopLossDollarAmount - (stopLossDollarAmount * .02),
                            ...
                            .then(() => {
                            this.logger.log(base_1.LogLevel.TRACE, `Successfully created an order for ${ticker.ticker} at ${ticker.price}`);
                        });
                    }
                    else {
                        this.logger.log(base_1.LogLevel.WARN, chalk_1.default.magentaBright(`${this.exchange.constructor.name} does not have enough buy power to purchase the configured amount of shares for ${ticker.ticker}`));
                        return;
                    }
                });
            }
            else {
                this.logger.log(base_1.LogLevel.TRACE, `Ticker: ${ticker} - Change Percent: ${changePercent} - Price: ${prevStockPrice}`);
            }
        });
    }
    ...
    getPrevStockPrice(ticker, amount = 15, unit = 'minutes', limit = 100) {
        ...
        return axios_1.default.get(`https://api.polygon.io/v2/ticks/stocks/trades/${ticker}/${nycTime.format('YYYY-MM-DD')}`, {
            ...
            .catch(err => {
            this.logger.log(base_1.LogLevel.ERROR, `Got error in getPrevStockPrice -> ${err}`);
            throw err;
        });
    }
