...
exports.StockBotOptionsValidationSchema = joi.object({
    ...
    exchange: joi.object().instance(exchange_2.AlpacasExchange).instance(exchange_1.PhonyExchange).required(),
...
class StockService extends base_1.Service {
    constructor(options) {
        super(options);
        this.fetchWork = () => {
            this.logger.log(base_1.LogLevel.INFO, `${this.constructor.name}#fetchWork():CALLED`);
        ...
        */
        this.preProcess = () => __awaiter(this, void 0, void 0, function* () {
            ...
            if (!marketIsOpen) {
                this.logger.log(base_1.LogLevel.INFO, 'Market is currently closed. Delaying next try by 5 minutes.');
                return bluebird_1.default.delay(5 * 60000).then(() => this.preProcess());
            } // else continue
            if (this.processables.length > 0) {
                ...
                //@ts-ignore
                this.processables = this.processables.filter((tkr) => !keys.includes(tkr.sym));
                ...
                return this.fetchWork()
                    ...
                    this.logger.log(base_1.LogLevel.INFO, `this.processables.length after filter = ${this.processables.length}`);
                    ...
                    if (!(this.processables.length > 0)) {
                        ...
                        if (this.processables.some((ticker) => !keys.includes(ticker.ticker))) {
                            this.logger.log(base_1.LogLevel.INFO, `The fetched tickers are all timed out. Waiting for all of the timed out tickers to resolve.`);
                            const pendingPromises = Array.from(this.datasource.timedOutTickers.values()).map(p => p.promise);
                            return Promise.all(pendingPromises)
                                .then(() => this.preProcess());
                        }
                        else {
                            this.logger.log(base_1.LogLevel.INFO, `this.processables.length = 0, return the backoff promise`);
                            return bluebird_1.default.delay(500).then(() => this.preProcess());
                        }
                    }
                    else {
                        this.logger.log(base_1.LogLevel.INFO, `Nothing in this.processables, instead retrying this.preProcess()`);
        ...
        this.exceptionHandler = (err) => {
            ...
            if (err.name === exception.UnprocessableTicker.name) {
                ...
                //Do nothing
                this.logger.log(base_1.LogLevel.INFO, `${this.constructor.name}#exceptionHandler - Received ServiceClosed error from Worker Process.`);
            }
            else {
    ...
    makeWorker(options) {
        ...
        //@ts-ignore
        return new this.mainWorker(Object.assign(Object.assign({}, options), { _preProcessor: this.preProcess, exceptionHandler: this.exceptionHandler, purchaseOptions: this.purchaseOptions, exchange: this.exchange, notification: this.notification, dataSource: this.datasource, dataStore: this.datastore }));
    }
    close() {
