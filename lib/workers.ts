    ...
    process(currTrade: TradeEvent): Promise<void> {
        ...
        .then((data: TradeEvent[]) => {
            if (!(data.length === 1)) {
                ...
                if (changePercentPerMinute > .039 && timeTaken >= 180) {
                    ...
                    .then((graphLink) => this.notification.notify({
                        ...
                        price: currTrade.p,
                        url: graphLink,
                        message: `Ticker ${currTrade.sym} has a rate of increase ${Number(changePercentPerMinute.toFixed(4)) * 100}% per minute.`,
                        additionaData: {
                            'Exchange': this.exchange.constructor.name,
                            'DataSource': this.datasource.constructor.name,
                    ...
                    // .then(() => {
                        ...
                        // })
                    // .then(() => {
                    //     this.logger.log(LogLevel.INFO, `Placed PAPER order for ${currTrade.sym}`);
                    // })
                    .then(() => {
