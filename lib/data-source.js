...
class YahooGainersDataSource extends DataSource {
    ...
    scrapeDatasource() {
        const scrapeUrl = 'https://finance.yahoo.com/gainers';
        return axios_1.default.get(scrapeUrl)
            .then((data) => {
                ...
                if (child.name === 'tr') {
                    if (child.firstChild.hasOwnProperty('children')) {
                        ...
                        try {
                            ...
                            throw new exceptions_1.InvalidData(`Error in ${this.constructor.name}._fetchHighIncreasedTickers(): innerError: ${err} -- ${JSON.stringify(err)}`);
                        }
...
class PolygonGainersLosersDataSource extends DataSource {
    ...
    scrapeDatasource() {
        const scrapeUrl = 'https://api.polygon.io/v2/snapshot/locale/us/markets/stocks/gainers';
        return Promise.all([axios_1.default.get(scrapeUrl + 'gainers'), axios_1.default.get(scrapeUrl + 'losers')])
            .then(((data) => {
            const tickers = [];
            try {
                ...
                throw new exceptions_1.InvalidData(`Error in ${this.constructor.name}.scrapeDatasource(): innerError: ${err} -- ${JSON.stringify(err)}`);
            }
            return tickers;
        }));
    }
