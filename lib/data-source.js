...
exports.PhonyDataSource = exports.PrometheusDataSource = exports.TwelveDataDataSource = exports.TwitterDataSource = exports.TwitterAccountType = exports.PolygonLiveDataSource = exports.PolygonGainersLosersDataSource = exports.YahooGainersDataSource = exports.DataSource = void 0;
...
(function (TwitterAccountType) {
    ...
    TwitterAccountType["TRACKER"] = "TRACKER";
...
class TwitterDataSource extends DataSource {
    constructor(options) {
        ...
        this._getLatestTweetsFromEachAccount = (input) => {
            console.log(`Incoming Data From Scrape: ${util_1.inspect(input)}`);
            let latestTweets = [];
        ...
        this._fetchLatestTimeline = (userId) => {
            ...
            return new Promise((resolve, reject) => {
                request.get(`https://api.twitter.com/2/users/${userId}/tweets?expansions=attachments.media_keys&media.fields=url&tweet.fields=in_reply_to_user_id`, this.twitterAccessToken, this.twitterAccessSecret, (e, data, res) => {
                    if (e) {
                        reject(e.data);
                    }
                    ...
                    let formatted;
                    //@ts-ignore
                    console.log(`data = ${!!data} -- data?.hasOwnProperty('data') = ${data.hasOwnProperty('data')}`);
                    console.log(`Raw Tweet Data: ${util_1.inspect(data)}`);
                    try {
                    ...
                    console.log(`Formatted Data: ${util_1.inspect(formatted)}`);
                    formatted.data.forEach((tweet, i) => {
                        var _a;
                        console.log(`Tweet From ${userId}`);
                        ...
                        if (tweet.in_reply_to_user_id && tweet.in_reply_to_user_id !== userId) {
                            console.log(`Tweet is a reply, does not count`);
                            return;
                        }
                        let tweetUrls = [];
                    ...
                    console.log(`Output Data: ${util_1.inspect(newObj)}`);
                    resolve(newObj);
                });
            });
        };
        ...
        this._redriveTweet = (accountId, tweet) => {
...
class PrometheusDataSource {
    ...
    getMetricByName(metric) {
        return axios_1.default.get(`http://${this.host}/api/v1/query`, {
            params: {
                'query': metric
            }
        })
            ...
            if (data.data.status === 'success') {
                ...
                throw new exceptions_1.InvalidDataError(`Error Type: ${data.data.errorType} -- Error: ${data.data.error}`);
            }
        });
    }
    close() {
