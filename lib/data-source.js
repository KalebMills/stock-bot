...
class TwitterDataSource extends DataSource {
    ...
    initialize() {
        if (!this.isMock) {
            ...
            this.clientStream.on('tweet', (data) => {
    ...
    */
    _processTweet(tweet) {
        ...
        const splitTweet = tweet.text.replace(/\n/g, '').split(" ");
        const ticker = splitTweet.filter(word => word.startsWith("$"));
        const hasTicker = ticker.length > 0 && this.tickerList.includes(ticker[0].substring(1).toUpperCase());
        if (hasTicker) {
            return Promise.resolve({
                account_name: tweet.user.screen_name,
                message: tweet.text,
                ticker: ticker[0],
                type: TwitterAccountType.UNKNOWN
            });
        }
        else {
            return Promise.resolve();
