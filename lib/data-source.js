...
exports.PhonyDataSource = exports.PolygonLiveDataSource = exports.MockEventEmitter = exports.PolygonGainersLosersDataSource = exports.YahooGainersDataSource = exports.DataSource = void 0;
...
class MockEventEmitter extends events_1.EventEmitter {
    constructor(options) {
        ...
        U.fetchTickersFromFile(path.join(__dirname, '..', 'resources', 'tickers.txt'))
            ...
            tickers.forEach(ticker => {
                this.tickers[ticker] = {
                    price: chance_1.default().integer({ min: 15, max: 3500 }) //Values are relatively arbitrary
                };
            });
     ...
     */
    startWorkerThread() {
        let workerId = chance_1.default().guid().substr(0, 5);
    ...
    execute(id) {
        ...
        if (this.workerThreads[id].active) {
            ...
            return Promise.resolve()
                ...
                .then(() => bluebird_1.default.delay(chance_1.default().integer({ min: 500, max: 1000 })))
     ...
     */
    getRandomTickerData() {
        let index = chance_1.default().integer({ min: 0, max: (Object.keys(this.tickers).length - 1) });
        let ticker = Object.keys(this.tickers)[index];
        let tickerObj = this.tickers[ticker];
        //Only allow for .01% change per event
        let price = chance_1.default().integer({ min: tickerObj.price - (tickerObj.price * (Math.random() * .01)), max: tickerObj.price + (tickerObj.price * (Math.random() * .01)) });
...
class PolygonLiveDataSource extends DataSource {
    constructor(options) {
        ...
        this._statusHandler = (data) => {
            switch (data.status) {
                ...
                case 'success':
                    ...
                    break;
                default:
                    this.logger.log(base_1.LogLevel.INFO, `Unknown status type ${data.status}`);
                    break;
            }
        };
        /*
        ...
        */
        ...
        this._tradeHandler = (data) => {
            this.logger.log(base_1.LogLevel.INFO, `${this.constructor.name}#data.length = ${this.data.length}`);
        ...
        this.emitter = options.mockEmitter || new events_1.EventEmitter();
    ...
    scrapeDatasource() {
        this.logger.log(base_1.LogLevel.INFO, `this.processables = ${this.data.length}`);
