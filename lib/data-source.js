        ...
        this._fetchLatestTimeline = (userId) => {
            ...
            return new Promise((resolve, reject) => {
                request.get(`https://api.twitter.com/2/users/${userId}/tweets?expansions=attachments.media_keys&media.fields=url&tweet.fields=in_reply_to_user_id`, this.twitterAccessToken, this.twitterAccessSecret, (e, data, res) => {
                    ...
                    this.logger.log(base_1.LogLevel.INFO, `Data from scrape: ${util_1.inspect(data)}`);
                    let formatted;
                    try {
                        if (data && (data === null || data === void 0 ? void 0 : data.hasOwnProperty('data'))) {
                            formatted = JSON.parse(data === null || data === void 0 ? void 0 : data.toString());
                        }
                        else {
                            throw new exceptions_1.InvalidDataError('Received no information in data');
                        }
                    }
                    catch (err) {
                        formatted = {
